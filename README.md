...
This archive contains only committed files from `HEAD`; it therefore excludes
`.git/`, untracked local state such as `.claude/settings.local.json`, build
output, caches, and temporary files. The release archive above is a different
artifact, generated by `scripts/build-release.py` from the trusted release
inventory.
